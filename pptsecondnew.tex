...
\item PLGP is vulnerable to Vampire attacks.
\item Consists of a topology discovery phase, followed by a packet forwarding phase.
...
\item PLGPa uses this packet history together with PLGP’s tree routing structure.
...
\item   B ----->  DEC(X)_{PA}  --->$ ENC(X,3,AB)_{PrB}$ == Y --->C


\item C -----> DEC(Y)_{PB} ---> $ENC(Y,2,ABC)_{PrC}$ == Z --> D


\item D -----> DEC(Z)_{PC}---> $ENC(Z,1,ABCD)_{PrD}$ ---->E
\end{itemize}
\end{frame}
